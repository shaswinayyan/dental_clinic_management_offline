  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
        <Typography.Title level={4} style={{ margin: 0, color: '#1e3a8a' }}>Inventory</Typography.Title>
      ...
      <Row gutter={16} style={{ marginBottom: 16 }}>
        <Col span={6}>
          <Card size="small" style={{ borderLeft: '4px solid #1d4ed8' }}>
            <Statistic title="Active Items" value={data?.totalItems ?? 0} valueStyle={{ color: '#1d4ed8', fontSize: 28 }} />
          </Card>
        </Col>
        <Col span={6}>
          <Card size="small" style={{ borderLeft: `4px solid ${(data?.lowStock.length ?? 0) > 0 ? '#dc2626' : '#16a34a'}`, cursor: 'pointer' }}>
        ...
        <Col span={6}>
          <Card size="small" style={{ borderLeft: `4px solid ${(data?.expiringSoon.length ?? 0) > 0 ? '#d97706' : '#16a34a'}` }}>
